...
def render_cost_report(out_dir: str, data: dict) -> str:
    ...
    L.append("# AWS 비용 리포트")
    L.append("")
    L.append(f"Account {data['account']} · {_ts()}  ")
    L.append(f"권한: `{data['caller_arn']}` (read-only)")
    L.append("")
    L.append("## 요약")
    L.append("")
    ...
    L.append("**주요 지표**")
    L.append("")
    L.append(f"- EC2 {len(ec2)}대 · idle (30일 avg CPU < 5%) {len(idle)}대")
    L.append(f"- ALB {len(albs)}개 · zero-traffic {len(zero_albs)}개 · low-traffic (<1k req/day) {len(low_albs)}개")
    L.append(f"- NAT GW {len(nats)}개 · zero-traffic {len(zero_nats)}개 · 고정비 합 ${len(nats)*0.059*730:.0f}/mo")
    ...
    if sp_rec_summary:
        L.append(f"- Compute SP 추천 적용 시 추가 절감 ${sp_rec_summary.get('EstimatedMonthlySavingsAmount', 0)}/mo ({float(sp_rec_summary.get('EstimatedSavingsPercentage', 0)):.1f}%)")
    L.append("")
    ...
    if zero_albs:
        L.append(f"| Zero-traffic ALB {len(zero_albs)}개 정리 | ${len(zero_albs)*22:.0f}+ | 낮음 (트래픽 재확인) |")
    if zero_nats:
        L.append(f"| Zero-traffic NAT GW {len(zero_nats)}개 제거 | ${len(zero_nats)*42:.0f} | 중간 (라우팅 변경) |")
    if free_snaps:
    ...
    if rds_idle:
        L.append(f"| IDLE RDS {len(rds_idle)}개 종료 (final snapshot 후) | ${sum(r.get('MonthlyUSD_approx') or 0 for r in rds_idle):.0f} | 낮음 (데이터 백업) |")
    if co["ec2_count"] + co["ebs_count"] > 0:
        L.append(f"| Compute Optimizer 추천 적용 ({co['ec2_count']} EC2 + {co['ebs_count']} EBS) | ${co['ec2_savings']+co['ebs_savings']:.0f} | 낮음 (추천별 risk 확인) |")
    L.append("")
    ...
    L.append("- `99-methodology.md` — 데이터 출처와 한계")
...
def render_problems(out_dir: str, data: dict) -> str:
    ...
    if sp_cov and float(sp_cov.get("CoveragePercentage", 0)) < 50:
        ...
        problems.append(("HIGH",
            f"Savings Plans 커버리지 {float(sp_cov.get('CoveragePercentage', 0)):.0f}%",
            "Account 전역",
            save,
    ...
    if co_savings > 0:
        problems.append(("LOW",
            ...
            "ap-northeast-2",
            co_savings,
            "AWS 자체 추천. Console 의 'Very Low' risk 부터 적용."))
...
def render_improvements(out_dir: str, data: dict) -> str:
    ...
    L.append(_ts())
    L.append("")
    L.append("각 액션은 예상 절감, 위험, 사전 검증, 롤백 방법을 포함한다.")
    L.append("")
    ...
    if gp2:
        ...
        L.append("- 위험: 없음 (in-place 변환, 다운타임 없음)")
    ...
    if zero_albs:
        ...
        L.append(f"- 절감: 약 ${len(zero_albs)*22}/mo")
        L.append("- 위험: 낮음 (잠시 미사용일 가능성)")
    ...
    if safe_term:
        ...
        L.append(f"- 절감: 약 ${sum(((e.get('AvgCPU') or 0)*5+10) for e in safe_term):.0f}/mo")
        L.append("- 위험: 낮음 (LB 미연결 + 사용 흔적 없음)")
        L.append("- 검증: Owner 태그 확인 → Slack 공지 → 7일 stop → terminate")
        L.append("")

    L.append("## Phase 2 — 1–2주 (소유자 확인 필요, 위험 중간)")
    L.append("")
    n = 0
    if zero_nats:
        n += 1
    ...
    if co["ec2_count"] + co["ebs_count"] > 0:
        ...
        L.append(f"- 절감: 약 ${co['ec2_savings']+co['ebs_savings']:.0f}/mo")
        L.append("- 위험: 낮음 (추천별 'Risk' 컬럼 확인 — Very Low → Low → Medium 순서로 적용)")
        L.append("")

    L.append("## Phase 3 — 전략 (1개월+)")
    L.append("")
    ...
    L.append("- Service Control Policy 로 Owner/Project/Environment 강제")
    L.append("- staging 환경에서 먼저 검증")
    L.append("")
    L.append("### 3. Cost Anomaly Detection + AWS Budgets (Terraform 에 이미 포함)")
